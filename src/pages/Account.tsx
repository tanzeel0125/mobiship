...
const Account = () => {
  ...
  const rows = [
    ...
    },
    {
      ...
      value: '•••• 5678',
      action: () => navigate('/account/billing'),
    },
    {
      icon: Settings,
    ...
    },
    {
      icon: Gift,
      label: 'Refer & Earn',
      value: 'Get $5 in shipping credit!',
  ...
  return (
    <div className="mobile-shell flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden">
      ...
      <main className="page-enter min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-4 py-6 pb-[calc(6.25rem+env(safe-area-inset-bottom,0px))]">
        {/* Profile header */}
          ...
          {editing ? (
            <div className="space-y-3 text-left">
              <input
                ...
                className="w-full h-10 px-4 rounded-lg border border-border text-sm outline-none focus:border-accent"
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleSave}
                  className="flex-1 h-10 bg-accent text-accent-foreground rounded-lg font-heading font-bold text-sm btn-press"
                >
                  Save
                </button>
                <button
                  onClick={() => setEditing(false)}
                  className="flex-1 h-10 border border-border rounded-lg text-sm"
                >
                  Cancel
                </button>
              </div>
            ...
            <>
              ...
              <button
                ...
                className="mt-2 text-xs border border-border rounded-lg px-3 py-1.5 inline-flex items-center gap-1"
              >
                <Pencil size={12} /> Edit
              </button>
            </>
          ...
          {rows.map((row, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {row.value && <p className="text-xs text-muted-foreground truncate">{row.value}</p>}
              </div>
              {row.pill && (
                <span
                  className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${notifications ? 'bg-success/10 text-success' : 'bg-muted text-muted-foreground'}`}
                >
                  {row.value}
                </span>
              )}
        ...
        <button
          ...
          className="btn-press mt-6 h-[52px] w-full rounded-xl bg-accent font-heading text-sm font-bold text-accent-foreground"
        >
          Log Out
        </button>
      </main>
